 ...
 *
 * @version $Id: FindPattern.java,v 1.1.1.1 1999/08/13 08:18:30 hugg Exp $
 * @author  <A HREF="http://www.inf.fu-berlin.de/~dahm">M. Dahm</A>
 * @see     Instruction
 * @see     InstructionList
 * @see     CodeConstraint
 */
public class FindPattern implements Constants {
  ...
  public final void reread() {
    ...
    for(int i=0; i < size; i++)
      buf[i] = makeChar(handles[i].getInstruction().getTag());
  ...
  private static final String pattern2string(String pattern, boolean make_string) {
    ...
    for(int i=0; i < pattern.length(); i++) {
      ...
      if(ch >= OFFSET) {
	if(make_string)
	  buf.append(OPCODE_NAMES[ch - OFFSET]);
	else
  ...
  StringBuffer buf;
     ...
     */
    appendPatterns(buf = new StringBuffer("("), IFEQ, LOOKUPSWITCH);
    buf.append('|');
    ...
    buf.append('|');
    buf.append(makeChar(LSTORE));
    buf.append(')');
    lstore_pattern = buf.toString();

    /* Make INVOKE pattern string
     */
    appendPatterns(buf = new StringBuffer("("), INVOKEVIRTUAL, INVOKEINTERFACE);
    buf.append(')');
    invoke_pattern = buf.toString();

    /* Make RETURN pattern string
     */
    appendPatterns(buf = new StringBuffer("("), IRETURN, RETURN);
    buf.append(')');
    return_pattern = buf.toString();

    /* Make IfInstruction pattern string
     */
    appendPatterns(buf = new StringBuffer("("), IFEQ, IF_ACMPNE);
    buf.append('|');
    buf.append(makeChar(IFNULL));
    buf.append('|');
    buf.append(makeChar(IFNONNULL));
    buf.append(')');

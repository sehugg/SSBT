...
//
// This is a JDK 1.2 compliant version of Hashtable.java
//
 ...
 * @modified    $Id: Hashtable.java,v 1.1.1.1 2000/02/17 08:14:12 hugg Exp $
 */
    ...
    implements Map, Cloneable, Serializable
{
     ...
     * elements exceeds the threshold, the Hashtable calls <pre>rehash()</pre> */
    ...
    /** counts the number of modifications this Hashtable has undergone; used by Iterators
     * to know when to throw ConcurrentModificationExceptions (idea ripped-off from
     * Stuart Ballard's AbstractList implementation) */
    int modCount;
    
    /**
     * construct a new Hashtable with the default capacity and the default load factor
     */
    public Hashtable()
    {
     ...
     *
     * <b>WARNING:  if a Hashtable is changed while an Enumeration is iterating
     * over it, the behavior of the Enumeration is undefined.  Use keySet().iterator()
     * if you want to be safe.</b>
     */
     ...
     *
     * <b>WARNING:  if a Hashtable is changed while an Enumeration is iterating
     * over it, the behavior of the Enumeration is undefined.  Use values().ieterator()
     * if you want to be safe.</b>
     */
     ...
     *
     * Note: this is one of the <i>old</i> Hashtable methods which does not like
     * null values; it throws NullPointerException if the supplied parameter is null.
     *
     ...
     * throw a NullPointerException when given a null argument
     * (Note:  Sun's implementation (JDK1.2beta4) <i>does</i> throw a 
     * NullPointerException when given a null argument, but this seems to go
     * against the Collections Framework specifications, so I have not
     * reproduced this behavior.  I have submitted a bug report to Sun on the mater,
     * but have not received any response yet (26 September 1998)
     *
     * @param      value       the value to search for in this Hashtable
     */
    ...
    /**
     * returns true if the supplied key is found in this Hashtable (strictly, if there
     * exists a key <pre>k</pre> in the Hashtable, such that <pre>k.equals(key)</pre>)
     *
    ...
    /** 
     * increases the size of the Hashtable and rehashes all keys to new array indices;
    ...
    /**
     * puts the supplied value into the Hashtable, mapped by the supplied key; neither 
     * the key nore the value is allowed to be <pre>null</pre>, otherwise a 
     * <pre>NullPointerException</pre> will be thrown
     *
    ...
    /** 
     * A private method with a semi-interesting history (it's at least two hours old
     * now); orginally, this functionality was in the public <pre>put()</pre> method,
     * but while searching (fruitlessly) on the JDC for some clarification of Javasoft's
     * bizarre Serialization documentation, I instead came across a JDK bug which had 
     * been fixed in JDK-1.2b3.  Extending Hashtable was a pain, because <pre>put()</pre>
     * was apparently being used internally by the class when the Hashtable was rehashed,
     * and this was causing odd behavior for people who had overridden <pre>put()</pre> 
     * in a Hashtable subclass.  Well, I was also calling <pre>put()</pre> internally, and
     * realized that my code would have the same problem. [No, I have never looked at the
     * Javasoft code; it was just the easiest thing to do].  So I put the real work in a 
     * private method, and I call <i>this</i> for internal use.  Except...not all the time.
     * What about <pre>putAll()</pre>?  Well, it seems reasonably clear from the Collections
     * spec that <pre>putAll()</pre> is <i>supposed</i> to call <pre>put()</pre>.  So, it
     * still does.  Confused yet?
     *
     * @param       key        the Hashtable key used to locate the value
     * @param       value      the value to be stored in the Hashtable
     */
  private Object internalPut(Object key, Object value)
    ...
    /**
     * removes from the Hashtable and returns the value which is mapped by the 
     * supplied key; if the key maps to nothing, then the Hashtable remains unchanged,
    ...
    /** 
     * returns a shallow clone of this Hashtable (i.e. the Hashtable itself is cloned, but
...
//     }
    ...
    /**
     * an inner class representing an Enumeration view of this Hashtable, providing 
     * sequential access to its elements; this implementation is parameterized to 
	 ...
	 *
	 * <b>NOTE: modifications to the backing Hashtable while iterating through an 
	 * Enumeration can result in undefined behavior, as the cursor may no longer be
	 * appropriately positioned</b>
	 */
